    ...
    match cli.command {
        ...
        Commands::Graph { action } => match action {
            GraphAction::Run {
                ...
                let yaml = std::fs::read_to_string(&abs_path).with_context(|| {
                    format!("failed to read graph file: {}", abs_path.display())
                })?;
                let graph_def: graph::GraphDef =
                    serde_yaml::from_str(&yaml).with_context(|| {
                        format!("failed to parse graph YAML: {}", abs_path.display())
                    })?;
            ...
            GraphAction::Validate { file } => {
                ...
                // topo_sort validates the DAG (no cycles, all deps exist).
