...
#[tokio::test]
async fn agent_detail_renders_for_known_agent() {
    use deskd::app::metrics::AgentDiskSample;
    let dir2 = tempfile::tempdir().unwrap();
    let home = dir2.path().to_string_lossy().to_string();
    std::fs::write(dir2.path().join("a.txt"), b"hello").unwrap();
    let sample = AgentDiskSample {
        agent: "kira".into(),
        home_dir: home.clone(),
        home_dir_bytes: Some(5 * 1024 * 1024),
    };
    let (state, _disp, _dir) =
        build_state_with_disk_seed(vec![], vec![sample], vec![("kira".into(), home.clone())]);
    let cookie = auth_cookie(&state);
    let app = router::build(state);
    let req = req_with_peer(
    ...
    assert!(body.contains("home dir"));
    assert!(body.contains("top "), "expected breakdown header in body");
}
